...
export const EditorNewsPostProvider = ({ children }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const createResponse = await fetch(`${process.env.API}/editor/news`, {
        method: 'POST',
        headers: {
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch(`${process.env.API}/editor/news`);
      if (!response.ok) {
  ...
  const updateItem = async (id, updatePost) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.API}/editor/news/${id.id}`, {
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`${process.env.API}/editor/news/${id}`, {
        method: 'DELETE'
      });
      const data = await response.json();
